export default class CardElement {
  constructor(cardData) {
    ...
    this.cardColors = ["--secondary-bg-color", "--bg-color-gradient-1", "--bg-color-gradient-2", "--bg-color-gradient-3", "--bg-color-gradient-4"];
    this.cardColorsIndex = 0;
  }
  ...
  create() {
    ...
    this.cardElement.querySelector('h3').onclick = (e) => {
      this.recolor(e.target);
    }
  ...
  recolor(target) {
    target.parentNode.style.setProperty('background', `var(${this.cardColors[this.cardColorsIndex]})`);
    this.cardColorsIndex++;
    if (this.cardColorsIndex >= this.cardColors.length) {
      this.cardColorsIndex = 0;
    }
  }
